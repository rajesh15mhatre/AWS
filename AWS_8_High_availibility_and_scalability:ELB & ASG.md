...
- Its allows us to validate traffic before visting instance. So first tarfiic goto to GLB then to atget group where it gets validated then to EC2 via GLB 
...
- means same user request goes to same EC2 instance
...
**Application Load Balancers** **and NLB**
• The “cookie” used for stickiness has an
expiration date you control
• Use case: make sure the user doesn’t lose user's
session data
...
- Hands ON
  ...
  - CHeck Stickiness cehckbox
    - Select suitable cookie
      - load balancer
      - App-based
        - give duration and name
    - Save changes
    - and check web app you will get same ip
    -  you can check cookie info in chrome delevelop tools under network tab
## 79. EL- Cross zone balancing  
- Cross zone balancing distribute traffic evently across all regisered instacnes in all AZ
